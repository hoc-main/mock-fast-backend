...
GROQ_FALLBACK_MODEL_2: str = "llama-3.1-8b-instant"  # second fallback — higher rate limits
GROQ_TIMEOUT: int = 15
...
_MAX_KB_EXAMPLES = 2       # reduced: 2 DB examples to keep payload small for gpt-oss-20b
_MAX_SESSION_HISTORY = 2   # reduced: last 2 exchanges only
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            for ans, q in all_rows[:_MAX_KB_EXAMPLES]:
                ...
                # Truncate transcript and feedback to keep payload small
                transcript_snippet = (ans.transcript or "")[:300]
                feedback_snippet = (ans.feedback or "")[:400]
                human_msg = (
                    f"QUESTION:\n{q.question_text[:200]}\n\n"
                ...
                ai_msg = (
                    f"FEEDBACK: {feedback_snippet}\n"
                    f"TIP: {(ans.tip or 'No specific tip provided.')[:200]}"
                )
...
def _build_input(
    ...
    # Truncate to keep total payload under ~2000 tokens for gpt-oss-20b
    return f"""QUESTION:
{question[:300]}

CANDIDATE'S ANSWER:
{candidate_answer[:600]}
...
def _call_chain_sync(human_input: str, cache_key: str) -> Optional[dict]:
    ...
    def _try_invoke(chain) -> Optional[dict]:
        output = chain.invoke({"input": human_input, "history": _get_full_history()})
    ...
    # ── Try primary model, rotating through all keys on rate limit ────────────
    import time as _time
    keys_tried = 0
    total_keys = len(GROQ_API_KEYS) or 1
    while keys_tried < total_keys:
        try:
            result = _try_invoke(_chain)
            if result:
            ...
            is_rate_limit = "rate_limit" in exc_str or "429" in exc_str or "too many" in exc_str
            is_too_large = "too large" in exc_str or "context" in exc_str or "payload" in exc_str or "413" in exc_str

            if is_too_large:
                # Payload too large — clear KB history and retry once with minimal context
                logger.warning(f"Payload too large for {GROQ_MODEL}, clearing KB history and retrying")
                _kb_examples.clear()
                ...
                try:
                    ...
                    pass
                break  # fall through to fallback models

            if is_rate_limit:
                keys_tried += 1
                if keys_tried < total_keys:
                    new_key = _rotate_key()
                    wait = min(1.0 * keys_tried, 3.0)
                    logger.warning(f"Rate limited, rotating to key #{_current_key_index + 1}, waiting {wait}s")
                    _time.sleep(wait)
                    try:
                        ...
                        pass
                    continue
                break  # all keys exhausted, fall through to fallbacks
            else:
                logger.warning(f"LangChain call failed: {exc}")
                break

    # ── Fallback 1: llama-3.3-70b-versatile ────────────────────────────────
    for fb_model in [GROQ_FALLBACK_MODEL, GROQ_FALLBACK_MODEL_2]:
        if not fb_model or fb_model == GROQ_MODEL:
            continue
        # Try each key for this fallback model
        for attempt in range(max(1, total_keys)):
            try:
                logger.info(f"Trying fallback model: {fb_model} (key #{_current_key_index + 1})")
                _build_chain(_get_current_key(), fb_model)
                result = _try_invoke(_chain)
                if result:
                    _add_to_history(human_input, result)
                    _cache_put(cache_key, result)
                    logger.info(f"LLM feedback OK via {fb_model} ({len(result['feedback'])} chars)")
                    # Restore primary model
                    try: _build_chain(_get_current_key(), GROQ_MODEL)
                    except Exception: pass
                    return result
                break
            except Exception as exc:
                exc_str = str(exc).lower()
                if ("rate_limit" in exc_str or "429" in exc_str) and attempt < total_keys - 1:
                    _rotate_key()
                    _time.sleep(min(1.5 * (attempt + 1), 4.0))
                    continue
                logger.warning(f"Fallback {fb_model} failed: {exc}")
                break
        # Restore primary model before trying next fallback
        try: _build_chain(_get_current_key(), GROQ_MODEL)
        except Exception: pass
