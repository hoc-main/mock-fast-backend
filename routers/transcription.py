        ...
        try:
            async with _dg_client.listen.v2.connect(
                ...
                def on_message(message) -> None:
                    nonlocal audio_paused  # needed if you ever set it here
                    ...
                    if event == "EndOfTurn":
                        # ✅ Fix 1: mutate the list, don't rebind it
                        final_parts.append(transcript)
                        asyncio.create_task(
                            websocket.send_text(json.dumps({"type": "final", "text": transcript}))
                        )
                        logger.debug(f"[session={session_id}] EndOfTurn: {transcript}")

                    elif event == "Update":
                        # ✅ Fix 1: slice-assign instead of rebinding
                        final_parts[:] = [transcript]
                        asyncio.create_task(
                            websocket.send_text(json.dumps({"type": "partial", "text": transcript}))
                        )
                ...
                try:
                    while True:
                        ...
                        if "bytes" in message:
                            # ✅ Fix 2: only forward audio if not paused
                            # (client sends silence/nothing after VAD fires,
                            ...
                            if text == "PAUSE":
                                # ✅ Fix 2: client signals VAD fired — stop
                                # forwarding audio so Flux sees silence and
                                # fires EndOfTurn naturally
                                audio_paused = True
                                logger.info(f"[session={session_id}] PAUSE received — waiting for EndOfTurn")
                                continue
